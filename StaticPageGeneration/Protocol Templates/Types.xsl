...
<xsl:template name="AllTypes">
	<xsl:for-each select="types/type[count(.//member) != 0]">
		<xsl:call-template name="OneType">
		</xsl:call-template>
	</xsl:for-each>
</xsl:template>

<!-- Generate the full documentation for one type. -->
<xsl:template name="OneType" match="type | enum">
	
	<xsl:result-document href="./Types/{@name}.html" method="html">
	<html>
	<head>
  ...
  <xsl:param name="text"/>
	...
	<a>
      ...
      <xsl:choose>
		...
		<xsl:choose>
		  <xsl:when test="$frame = 'frameType'"></xsl:when>
		  <xsl:when test="$frame = 'frameEnum'"></xsl:when>
